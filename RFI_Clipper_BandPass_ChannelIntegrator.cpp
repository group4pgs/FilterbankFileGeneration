...
int main(int argc, char * argv [])
{
  ...
  {  
    ...
    **/
    ...
    for (unsigned s = 0; s < nSubbands; ++s) 
    {
      ...
      {
        int binLocal = s*nChannels +c;
        _channelIntegratorMean[binLocal] /= nSamples;
        _channelIntegratorRMS[binLocal] = sqrt(_channelIntegratorSumSq[binLocal]/nSamples - std::pow(_channelIntegratorMean[binLocal],2)); 
      }
    }
    float meanOfChannelMeanIntergrators = std::accumulate(_channelIntegratorMean.begin(), _channelIntegratorMean.end(), 0.0)/(nChannels*nSubbands);
    double accum = 0.0;
    std::for_each (std::begin(_channelIntegratorMean), std::end(_channelIntegratorMean), [&](const double d) {accum += (d - meanOfChannelMeanIntergrators) * (d - meanOfChannelMeanIntergrators);});
    double stdevOfChannelMeanIntegrators = sqrt(accum / (_channelIntegratorMean.size()-1));

    float meanOfChannelRMSIntergrators  = std::accumulate(_channelIntegratorRMS.begin(), _channelIntegratorRMS.end(), 0.0)/(nChannels*nSubbands);
    double accum2 = 0.0;
    std::for_each (std::begin(_channelIntegratorRMS), std::end(_channelIntegratorRMS), [&](const double d) {accum2 += (d - meanOfChannelRMSIntergrators) * (d - meanOfChannelRMSIntergrators);});
    double stdevOfChannelRMSIntegrators = sqrt(accum2 / (_channelIntegratorRMS.size()-1));
    float _channelFactor = 3.3;
    for (unsigned s = 0; s < nSubbands; ++s) 
    {
      for (unsigned c = 0; c < nChannels; ++c) 
      {
        int binLocal = s*nChannels +c;
        if ((_channelIntegratorMean[binLocal] -meanOfChannelMeanIntergrators > (_channelFactor*stdevOfChannelMeanIntegrators) || _channelIntegratorRMS[binLocal] - meanOfChannelRMSIntergrators  > (_channelFactor*stdevOfChannelRMSIntegrators)) && binLocal!=nBins-1)
        {
          ++_badChannels;
          int temp = binLocal+1;
          while (_channelIntegratorMean[temp] - meanOfChannelMeanIntergrators > (_channelFactor*stdevOfChannelMeanIntegrators) || _channelIntegratorRMS[temp] - meanOfChannelRMSIntergrators  > (_channelFactor*stdevOfChannelRMSIntegrators)) temp +=1;
          for (int i = binLocal; i<averageSamplestoReadFromFile; i += nChannels )
          {
            int diff = temp-binLocal;
            I[i] = I[i+diff];
          }
        }
        else if ((_channelIntegratorMean[binLocal] - meanOfChannelMeanIntergrators > (_channelFactor*stdevOfChannelMeanIntegrators) || (_channelIntegratorRMS[binLocal] - meanOfChannelRMSIntergrators) > (_channelFactor*stdevOfChannelRMSIntegrators)) && binLocal == nBins-1)
        {
          ++_badChannels;
          for (int i = binLocal; i<averageSamplestoReadFromFile; i += nChannels )
          {
 ...
 // std::cout << "The number of bad channels replaced are: " << _badChannels << endl;
 tend = time(0); 
